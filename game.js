 ...
 * EXTENSIONS
 ***********************************************************************/
/* ButtonExtension by RedAnt */

function __Button() {
__instance_init__(this, Button, null, 1, 0, null, 1, 0);
this.on_creation = function() {
with(this) {
this._font={bold:0,italic:0,size:20,family:"Arial"};
this._text="undefined";
this.inactiveColor={red:255,green:0,blue:0};
this.hoverColor={red:0,green:255,blue:0};
this.pressedColor={red:0,green:0,blue:255};
this._color=this.inactiveColor;
this._measurer=document.createElement("canvas").getContext("2d");
this.onClick=function() {
	alert("On click function.");
}
this._refreshMeasures=function() {
	var newMeasurerFont="";
	newMeasurerFont+=(this._font.bold)?"bold ":"";
	newMeasurerFont+=(this._font.italic)?"italic ":"";
	newMeasurerFont+=this._font.size + "px " + this._font.family;
	this._measurer.font=newMeasurerFont;
	this._width=this._measurer.measureText(this._text).width;
	this._height=this._font.size;
}
var_override(this,"width",function() { return this._width },undefined);
var_override(this,"height",function() { return this._height },undefined);
var_override(this,"font",
	function() {
		return this._font;
	},
	function(newFont) {
		this._font=newFont;
		this._refreshMeasures();
	});
var_override(this,"text",
	function() {
		return this._text;
	},
	function(newText) {
...
with(this) {
this._color=this.inactiveColor;
if(mouse_x > this.x && mouse_x < (this.x+this._width)) {
	if(mouse_y > this.y && mouse_y < (this.y+this._height)) {
		if(mouse_down) {
			this._color=this.pressedColor;
		} else {
			this._color=this.hoverColor;
		}
...
__handle_sprite__(this);
with(this) {
draw_set_font(this._font);
draw_set_color(this._color.red,this._color.green,this._color.blue);
draw_text(this.x,this.y,this._text);
}
}
};
}; var Button = new __Button();
 ...
 ***********************************************************************/
function __sprBlue() { 
__sprite_init__(this, sprBlue, 32, 32, 16, 16, 'Box', 16, 0, 32, 0, 32, ['img/sprBlue_0.png']);
}; var sprBlue = new __sprBlue();

function __sprPurple() { 
__sprite_init__(this, sprPurple, 32, 32, 16, 16, 'Box', 16, 0, 32, 0, 32, ['img/sprPurple_0.png']);
}; var sprPurple = new __sprPurple();

function __sprRed() { 
__sprite_init__(this, sprRed, 32, 32, 16, 16, 'Box', 16, 0, 32, 0, 32, ['img/sprRed_0.png']);
}; var sprRed = new __sprRed();

function __sprGreen() { 
__sprite_init__(this, sprGreen, 32, 32, 16, 16, 'Box', 16, 0, 32, 0, 32, ['img/sprGreen_0.png']);
}; var sprGreen = new __sprGreen();

function __sprYellow() { 
__sprite_init__(this, sprYellow, 32, 32, 16, 16, 'Box', 16, 0, 32, 0, 32, ['img/sprYellow_0.png']);
}; var sprYellow = new __sprYellow();

function __sprNone() { 
__sprite_init__(this, sprNone, 32, 32, 16, 16, 'Box', 16, 0, 32, 0, 32, ['img/sprNone_0.png']);
}; var sprNone = new __sprNone();

function __sprScroolArrowIdle() { 
__sprite_init__(this, sprScroolArrowIdle, 24, 24, 12, 12, 'Box', 12, 0, 24, 0, 24, ['img/sprScroolArrowIdle_0.png']);
}; var sprScroolArrowIdle = new __sprScroolArrowIdle();

function __sprScrollArrowPressed() { 
__sprite_init__(this, sprScrollArrowPressed, 24, 24, 12, 12, 'Box', 12, 0, 24, 0, 24, ['img/sprScrollArrowPressed_0.png']);
}; var sprScrollArrowPressed = new __sprScrollArrowPressed();

function __sprScrollArrowHover() { 
__sprite_init__(this, sprScrollArrowHover, 24, 24, 12, 12, 'Box', 12, 0, 24, 0, 24, ['img/sprScrollArrowHover_0.png']);
}; var sprScrollArrowHover = new __sprScrollArrowHover();

function __sprSolved() { 
__sprite_init__(this, sprSolved, 300, 96, 150, 48, 'Box', 150, 0, 300, 0, 96, ['img/sprSolved_0.png']);
}; var sprSolved = new __sprSolved();
 ...
 * FONTS
 ***********************************************************************/
function __buttonFont() { 
__font_init__(this, buttonFont, 'Copperplate Gothic', 25, 1, 0)}; var buttonFont = new __buttonFont();

function __textFont() { 
__font_init__(this, textFont, 'Copperplate Gothic', 20, 0, 0)}; var textFont = new __textFont();
 ...
 ***********************************************************************/
function __Tile() {
__instance_init__(this, Tile, null, 1, 0, sprNone, 1, 0);
this.on_creation = function() {
with(this) {
Tile.sprites=[sprBlue,sprGreen,sprPurple,sprRed,sprYellow];

var_override(this,"row",
	function() {
		return this._row;
	},
	function(newRow) {
...
this.adjustColor=function(rubicrossStateInstance) {
	if(this._row != undefined && this._column != undefined) {
		this.sprite_index=Tile.sprites[rubicrossStateInstance.getData(this._row,this._column)];
	} else {
		this.sprite_index=sprNone;
	}
}
}
};
...
function __ScrollArrow() {
__instance_init__(this, ScrollArrow, null, 1, 0, sprScroolArrowIdle, 1, 1);
this.on_creation = function() {
with(this) {
this.lineIndex=undefined;
this.scrollDirection=undefined;
this.rubicrossStateInstance=undefined;

this.onClick=function() {
	if(this.scrollDirection==RubicrossState.UP || this.scrollDirection==RubicrossState.DOWN) {
		this.rubicrossStateInstance.scrollVertically(this.lineIndex,this.scrollDirection);
	} else {
		//left,right and all other wrong directions
		this.rubicrossStateInstance.scrollHorizontally(this.lineIndex,this.scrollDirection);
	}
	movesMade++;
	refreshTileColors(tiles,mainState);
}
}
};
this.on_destroy = on_destroy_i;
this.on_step = function() {
with(this) {
this.sprite_index=sprScroolArrowIdle;
if(abs(mouse_x-this.x) <= this.sprite_index.width/2 && abs(mouse_y-this.y) <= this.sprite_index.height/2) {
	this.sprite_index=sprScrollArrowHover;
	
	if(mouse_down) {
		this.sprite_index=sprScrollArrowPressed;
	}
...
this.on_animationend = on_animationend_i;
this.on_draw = on_draw_i;
}; var ScrollArrow = new __ScrollArrow();

function __RotationArrow() {
__instance_init__(this, RotationArrow, null, 1, 0, sprScroolArrowIdle, 1, 2);
this.on_creation = function() {
with(this) {
this.ring=undefined;
...
this.onClick=function() {
	this.rubicrossStateInstance.rotate(this.ring,this.rotationDirection);
	movesMade++;
	refreshTileColors(tiles,mainState);
}
}
...
this.on_animationend = on_animationend_i;
this.on_draw = on_draw_i;
}; var RotationArrow = new __RotationArrow();

function __Scrambler() {
__instance_init__(this, Scrambler, null, 0, 0, null, 0, 3);
this.on_creation = function() {
with(this) {
...
this.on_step = function() {
with(this) {
if(this.scrambleNumber>0 && (new Date().getTime() - this.lastScrambleTimestamp)>100) {
	this.scrambleNumber--;
	this.lastScrambleTimestamp=new Date().getTime();
	mainState.doRandomMove();
	refreshTileColors(tiles,mainState);
}
}
};
...
this.on_animationend = on_animationend_i;
this.on_draw = on_draw_i;
}; var Scrambler = new __Scrambler();

function __MovesDrawer() {
__instance_init__(this, MovesDrawer, null, 1, 0, null, 0, 4);
...
draw_text(room_width - 100,8,"Moves: " + movesMade);
}
}
};
}; var MovesDrawer = new __MovesDrawer();

function __SolvedBanner() {
__instance_init__(this, SolvedBanner, null, 0, -1, sprSolved, 0, 6);
this.on_creation = on_creation_i;
this.on_destroy = on_destroy_i;
this.on_step = on_step_i;
this.on_end_step = on_end_step_i;
...
this.on_animationend = on_animationend_i;
this.on_draw = on_draw_i;
}; var SolvedBanner = new __SolvedBanner();
 ...
 * SCENES
 ***********************************************************************/
function __mainScene() { 
this.tiles = [
];
this.objects = [
[{o:MovesDrawer, x:280, y:20}],
[{o:SolvedBanner, x:190, y:320}]];
this.start = function() {
__room_start__(this, mainScene, 380, 380, 30, 0, 0, 0, null, 0, 0, 0, 380, 380, null, 50, 50);
...
for(var i=0;i<9;i++)
	for(var j=0;j<9;j++)
		if((i>2 && i<6) || (j>2 && j<6)) {
			var newX=room_width/2+(j-4)*32;
			var newY=room_height/2+(i-4)*32;
			var newTile=instance_create(newX,newY,Tile);
			tiles.push(newTile);
			newTile.row=i;
			newTile.column=j;
		}
...
//generate scroll arrows 
for(var i=0;i<3;i++) {
	for(var j=-1;j<2;j+=2) {
		//horizontal
		var newInstance=instance_create(room_width/2+5*32*j,room_height/2+(i-1)*32,ScrollArrow);
		newInstance.rubicrossStateInstance=mainState;
		newInstance.lineIndex=i;
		if(j==-1) {
			newInstance.image_angle=180;
			newInstance.scrollDirection=RubicrossState.LEFT;
		} else {
			newInstance.scrollDirection=RubicrossState.RIGHT;
		}
			
		//vertical
		newInstance=instance_create(room_width/2+(i-1)*32,room_height/2+5*32*j,ScrollArrow);
		newInstance.rubicrossStateInstance=mainState;
		newInstance.lineIndex=i;
		if(j==-1) {
			newInstance.image_angle=90;
			newInstance.scrollDirection=RubicrossState.UP;
		} else {
			newInstance.image_angle=270;
			newInstance.scrollDirection=RubicrossState.DOWN;
		}
	}
}
...
for(var i=0;i<3;i++) {
	for(var j=-1;j<2;j+=2) {
		//top
		var newInstance=instance_create(room_width/2+j*(4-i)*32,room_height/2-32*2,RotationArrow);
		newInstance.rubicrossStateInstance=mainState;
		newInstance.ring=2-i;
		newInstance.image_angle=90;
		if(j==-1) {
			newInstance.rotationDirection=RubicrossState.RIGHT;
		} else {
			newInstance.rotationDirection=RubicrossState.LEFT;
		}
		//bottom
		var newBottomInstance=instance_create(newInstance.x,newInstance.y,RotationArrow);
		newBottomInstance.y+=4*32;
		newBottomInstance.image_angle=270;
		newBottomInstance.ring=newInstance.ring;
		newBottomInstance.rubicrossStateInstance=mainState;
		if(j==-1) {
			newBottomInstance.rotationDirection=RubicrossState.LEFT;
		} else {
			newBottomInstance.rotationDirection=RubicrossState.RIGHT;
		}
	}
}
//scrambler
scrambler=instance_create(0,0,Scrambler);

//scramble button
var newButton=instance_create(8,8,Button);
newButton.text="Scramble";
newButton.font=buttonFont;
newButton.onClick=function() {
	scrambler.scrambleNumber=15;
	movesMade=0;
	instance_list(SolvedBanner)[0].visible=false;
	scrambled=true;
}

//reset button
newButton=instance_create(8,8+newButton.font.size,Button);
newButton.text="Reset";
newButton.font=buttonFont;
newButton.onClick=function() {
	RubicrossState.call(mainState);
	refreshTileColors(tiles,mainState);
	movesMade=0;
	scrambled=false;
	instance_list(SolvedBanner)[0].visible=false;
...
tu_scenes.push(mainScene);
tu_room_to_go = mainScene;


/***********************************************************************
 * CUSTOM GLOBAL VARIABLES
 ***********************************************************************/
var mainState=new RubicrossState();
var tiles=[];
var scrambler;
var movesMade=0;
var scrambled=false;
...
function RubicrossState() { 
RubicrossState.LEFT="left";
RubicrossState.RIGHT="right";
RubicrossState.UP="up";
RubicrossState.DOWN="down";
...
var_override(this,"solved",
	function() {
		function squareSame(row,column) {
			var same=true;
			var reference=this._data[row][column];
			
			for(var i=0;i<9;i++) {
				var current=this._data[row+floor(i/3)][column+i%3];
				same = same && (reference==current);
			}
			return same;
		}
		...
		for(var i=0;i<9;i+=3)
			for(var j=0;j<9;j+=3)
				solved = solved && squareSame.call(this,i,j);
...
this.scrollHorizontally=function(row,scrollDirection) {
	if(row>2 || row<0)
		throw new Error("Cannot horizontally scroll row " + row);
	...
	for(var i=0;i<9;i++) {
		...
		if(scrollDirection==RubicrossState.LEFT)
			newIndex=i+3;
		else if(scrollDirection==RubicrossState.RIGHT)
			newIndex=i-3;
		else
			throw new Error("Wrong scrolling direction - " + scrollDirection);
		
		if(newIndex<0 || newIndex>8)
			newIndex=(9+newIndex)%9;
			
		newArray[i]=this._data[actualRow][newIndex]
	}
	
	this._data[actualRow]=newArray;
}

this.scrollVertically=function(column,scrollDirection) {
	if(column<0 || column>2)
		throw new Error("Cannot vertically scroll column " + column);
		
	var actualColumn=column+3;
	var newArray=new Array(9);
	
	for(var i=0;i<9;i++) {
		var newIndex;
	
		if(scrollDirection==RubicrossState.DOWN)
			newIndex=i-3;
		else if(scrollDirection==RubicrossState.UP)
			newIndex=i+3;
		else
			throw new Error("Wrong scrolling direction - " + scrollDirection);
		...
		newArray[i]=this._data[newIndex][actualColumn];
	}
	
	for(var i=0;i<9;i++)
		this._data[i][actualColumn]=newArray[i];
}

this.rotate=function(ring,rotationDirection) {
	if(ring>2 || ring<0)
		throw new Error("Cannot rotate ring " + ring);

	var startLocations;
	//used to solve problems with logical direction and array direction
	var repairSwap;
	
	if(rotationDirection==RubicrossState.LEFT) {
		//list of starts of triples which need to be swaped
		startLocations=[[3,2-ring],[2-ring,3],[3,6+ring],[6+ring,3]];
		//array of location pairs
		repairSwap=[[[3,2-ring],[5,2-ring]],[[3,6+ring],[5,6+ring]]];
	} else if(rotationDirection==RubicrossState.RIGHT) {
		startLocations=[[3,2-ring],[6+ring,3],[3,6+ring],[2-ring,3]];
		//array of location pairs
		repairSwap=[[[2-ring,3],[2-ring,5]],[[6+ring,3],[6+ring,5]]];
	} else
		throw new Error("Wrong rotation direction - " + rotationDirection);
	
	for(var i=0;i<3;i++) {
		var swapTmp;
		for(var j=0;j<3;j++) {
			var firstSwapRow=startLocations[i][0];
			var firstSwapColumn=startLocations[i][1];
			var secondSwapRow=startLocations[i+1][0];
			var secondSwapColumn=startLocations[i+1][1];
			
			if(i%2==0) {
				//first swap is in column and second is in row
				firstSwapRow+=j;
				secondSwapColumn+=j;
			} else {
				//first swap is in row and second is in column
				firstSwapColumn+=j;
				secondSwapRow+=j;
			}
			
			swapTmp=this._data[firstSwapRow][firstSwapColumn];
			this._data[firstSwapRow][firstSwapColumn]=this._data[secondSwapRow][secondSwapColumn];
			this._data[secondSwapRow][secondSwapColumn]=swapTmp;
		}
	}
	
	//repair swaps
	for(var i=0;i<2;i++) {
		var swapTmp=this._data[repairSwap[i][0][0]][repairSwap[i][0][1]];
		this._data[repairSwap[i][0][0]][repairSwap[i][0][1]]=this._data[repairSwap[i][1][0]][repairSwap[i][1][1]];
		this._data[repairSwap[i][1][0]][repairSwap[i][1][1]]=swapTmp;
	}
}

this.getData=function(row,column) {
	if(this._data[row][column]==9)
		throw new Error("Access to field ("+row+","+column+") denied");
	else
		return this._data[row][column];
}

this.doRandomMove=function() {
	var functionToCall=choose(0,1,2);
	
	if(functionToCall==0)
		this.rotate(choose(0,1,2),choose(RubicrossState.LEFT,RubicrossState.RIGHT));
	else if(functionToCall==1)
		this.scrollHorizontally(choose(0,1,2),choose(RubicrossState.LEFT,RubicrossState.RIGHT));
	else
		this.scrollVertically(choose(0,1,2),choose(RubicrossState.UP,RubicrossState.DOWN));
}
}
function refreshTileColors(tiles,rubicrossStateInstance) { 
for(var i=0;i<tiles.length;i++) {
	tiles[i].adjustColor(rubicrossStateInstance);
}
	
instance_list(SolvedBanner)[0].visible=scrambled && mainState.solved && movesMade>0;
}
